    ...
    //vars
const emojis = ["😊", "😂", "😍", "😒", "😘", "😁", "😢", "😜", "😎", "😉", "😊", "😂", "😍", "😒", "😘", "😁", "😢", "😜", "😎", "😉"];
const buttonsBase:Element[] = [...document.querySelectorAll(".emojiBtn")];
let buttons:Element [] = [];
...
function chooseCard(event:Event) {
    ...
    numtimesclicked++;
    const emoji = event.target as HTMLButtonElement;
...
function checkForMatch(emojicard1: Element, emojicard2: Element) {
    
    const emj1 = emojicard1 as HTMLButtonElement;
    const emj2 = emojicard2 as HTMLButtonElement;
...
function displayMessage(message: string) {
    ...
    setTimeout(function () {
       let messagedisplay:Element = messagebox; // added let and datatype element??
    }, 1000);
}
